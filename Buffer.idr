...
record Buffer where
  ...
  rawdata : Ptr -- let Idris run time manage the memory
...
rawSize b = foreign FFI_C "idris_getBufferSize" (Ptr -> IO Int) (rawdata b)
...
setByte b loc val
    = foreign FFI_C "idris_setBufferByte" (Ptr -> Int -> Bits8 -> IO ())
              (rawdata b) loc val
...
setInt b loc val
    = foreign FFI_C "idris_setBufferInt" (Ptr -> Int -> Int -> IO ())
              (rawdata b) loc val
...
setDouble b loc val
    = foreign FFI_C "idris_setBufferDouble" (Ptr -> Int -> Double -> IO ())
              (rawdata b) loc val
...
setString b loc val
    = foreign FFI_C "idris_setBufferString" (Ptr -> Int -> String -> IO ())
              (rawdata b) loc val
...
copyData src start len dest loc 
    = foreign FFI_C "idris_copyBuffer" (Ptr -> Int -> Int -> Ptr -> Int -> IO ())
...
getByte b loc
    = foreign FFI_C "idris_getBufferByte" (Ptr -> Int -> IO Bits8)
              (rawdata b) loc 
...
getInt b loc
    = foreign FFI_C "idris_getBufferInt" (Ptr -> Int -> IO Int)
              (rawdata b) loc 
...
getDouble b loc
    = foreign FFI_C "idris_getBufferDouble" (Ptr -> Int -> IO Double)
              (rawdata b) loc 
...
getString b loc len
    = do MkRaw str <- foreign FFI_C "idris_getBufferString" (Ptr -> Int -> Int -> IO (Raw String))
                          (rawdata b) loc len
         pure str
...
do_fopen f m
   = foreign FFI_C "_fileOpen" (String -> String -> IO Ptr) f m

export
_openFile : (f : String) -> (m : String) -> IO (Either FileError File)
_openFile f m = do h <- do_fopen f m
                   if !(nullPtr h)
...
readBufferFromFile (FHandle f) buf max
    = do numread <- foreign FFI_C "idris_readBuffer" (Ptr -> Ptr -> Int -> Int -> IO Int)
                       f (rawdata buf) (location buf) max
...
writeBufferToFile (FHandle h) buf max
    = do let maxwrite = size buf - location buf
         ...
         foreign FFI_C "idris_writeBuffer" (Ptr -> Ptr -> Int -> Int -> IO ())
